...
[<TypeProvider>]
type MSSQLProvider(config) as this =
    inherit TypeProviderForNamespaces()
    let sqlRuntimeInfo = SqlRuntimeInfo(config)
    let ns = "FSharp.Data.Sql";   

    let paramSqlType = ProvidedTypeDefinition(sqlRuntimeInfo.RuntimeAssembly, ns, "MSSQLProvider", Some(typeof<obj>), HideObjectMethods = true)
    ...
    do paramSqlType.DefineStaticParameters([conString;connStringName;individualsAmount;optionTypes], fun typeName args -> 
