...
[<TypeProvider>]
type PostgreProvider(config) as this =
    inherit TypeProviderForNamespaces()
    let sqlRuntimeInfo = SqlRuntimeInfo(config)
    let ns = "FSharp.Data.Sql";   

    let paramSqlType = ProvidedTypeDefinition(sqlRuntimeInfo.RuntimeAssembly, ns, "PostgreProvider", Some(typeof<obj>), HideObjectMethods = true)
    ...
    do paramSqlType.DefineStaticParameters([conString;connStringName;resolutionPath;individualsAmount;optionTypes;owner], fun typeName args -> 
