...
type internal ISqlProvider =
    ...
    /// Accepts a full clr type name and returns the relevant value from the SqlDbType enum
    abstract ClrToEnum : (string -> SqlDbType option) with get
    /// Accepts sql datatype name and returns the relevant value from the SqlDbType enum
    ...
    /// have a single non-composite primary ket
    ...
    /// Returns cntraint information for a given table, returning two lists of relatioships, where
    ...
    /// the other parameters are the base tbale alias, the base table, and a dictionary containing 
