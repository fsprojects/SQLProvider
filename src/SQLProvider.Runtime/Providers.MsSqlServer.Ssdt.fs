...
module MSSqlServerSsdt =
    ...
    let findDacPacFile (dacPacPath: string) =
        ...
        | None ->
            ...
            sb.AppendLine($"Unable to find .dacpac file. Search path includes executing assembly, configured ssd path, entry assembly, and the environment variable '%s{DACPAC_SEARCH_PATH_ENV_VAR_NAME}'.") |> ignore
